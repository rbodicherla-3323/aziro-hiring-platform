...
async function startScreenCaptureMonitor() {
    ...
    try {
        ...
        screenCaptureReady = true; screenCaptureUnavailableLogged = false; lastScreenCaptureRejectReason = "";
...
function getProctoringRequirementMessage() {
    if (!screenCaptureReady && supportsDisplayCapture()) return "Share Entire Screen permission is required to continue the test.";
...
async function startWebcamRecording() {
    if (!isExamPath() || !hasUsableWebcamStream(webcamStream)) return;
...
function stopWebcamPreview(options = {}) {
    ...
    if (webcamStream) { releaseWebcamStream(webcamStream); webcamStream = null; }
}
...
function setupExamProctoring() {
    ...
    startPeriodicScreenshotCapture(); startContentChangeDetection(); startMultiMonitorDetection(); setupExternalDeviceDetection();
    ...
    window.addEventListener("beforeunload", () => {
        ...
        stopWebcamPreview({ final: true }); stopScreenCaptureMonitor(); stopPeriodicScreenshotCapture(); stopContentChangeDetection(); stopMultiMonitorDetection(); teardownExternalDeviceDetection();
