...
def access_management_revoke():
    ...
    # Notify admin (even if missing).
    notify_ok, notify_err = _send_access_revoked_admin_email(email, existed=existed)

    if existed:
        if notify_ok:
            flash(f"Revoked access for {email}. Admin notified.", "warning")
        else:
            flash(f"Revoked access for {email}, but notification failed: {notify_err}", "warning")
    else:
        if notify_ok:
