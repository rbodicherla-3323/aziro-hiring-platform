...
def _session_scope_by_email(test_entries) -> dict[str, set[str]]:
    ...
    for entry in test_entries or []:
        ...
        if not email_key:
            continue
        # test_entries are sorted latest-first; scope fallback to latest row per email only
...
def reports():
    ...
    # HR-scoped generated tests for selected period.
    ...
    # When searching, include org-wide DB candidate matches too (even if they are
    # outside this HR's generated list) so search can access full database scope.
